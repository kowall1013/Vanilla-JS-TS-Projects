...
const display = calculator.querySelector(".calculator__display") as HTMLElement;
...
const calculate = (
  operator: string,
  firstValueArg: string,
  secondValueArg: string
) => {
...
function handleClearKey(calculator: HTMLElement, button: HTMLButtonElement) {
...
function handleNumberKeys(calculator: HTMLElement, button: HTMLButtonElement) {
  const { previousButtonType } = calculator.dataset;
  // @ts-ignore
  const { key }: { key: string } = button.dataset;
...
function handleDecimalKey(calculator: HTMLElement) {
  const displayValue = getDisplayValue();
  const { previousButtonType } = calculator.dataset;
...
function handleOperatorKeys(
  ...
  if (
    ...
    if (secondValue === undefined || secondValue === null) return;
    ...
    if (displayValue) calculator.dataset.firstValue = displayValue;
  }

  calculator.dataset.operator = button.dataset.key;
}

function handleEqualKey(calculator: HTMLElement) {
  const displayValue = getDisplayValue();
...
calculatorButtonsDiv.addEventListener("click", (event) => {
  ...
  const operatorKeys = [...calculatorButtonsDiv.children].filter((button) => {
    const buttonAsHTMLElement = button as HTMLButtonElement;
...
calculatorButtonsDiv.addEventListener("keydown", (event) => {
  ...
  const button = calculator.querySelector(`[data-key="${key}"]`) as HTMLElement;
  if (!button) return;
