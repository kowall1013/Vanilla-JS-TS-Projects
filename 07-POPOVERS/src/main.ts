 ...
 */
const getPopover = (popoverTrigger: HTMLElement) => {
 ...
 * @returns {HTMLElement}
 */
const createPopover = (popoverTrigger: HTMLElement) => {
 ...
 */
const calculatePopoverPosition = (popoverTrigger: Element, popover: HTMLElement) => {
...
const hidePopover = (popover: Element) => {
...
const getKeyboardFocusableElement = (element: Element) => {
  return [...element.querySelectorAll(
    'a, button, input, textarea, select, [tabindex]:not([tabindex="-1"])'
  )]
}

const getPopoverTrigger = (popover: HTMLElement) => {
...
popoverTriggers.forEach(popoverTrigger => {
  const popoverTriggerAsHTMLElement = <HTMLElement>popoverTrigger
  const popover = <HTMLElement>getPopover(popoverTriggerAsHTMLElement) || createPopover(popoverTriggerAsHTMLElement)
...
document.addEventListener('click', event => {
  ...
  const popoverTrigger = <HTMLElement>target.closest('.popover-trigger')
...
document.addEventListener('keydown', event => {
  ...
  assert(popover !== null, 'popover is null')
  const focusables = getKeyboardFocusableElement(popover) as HTMLElement[]
...
document.addEventListener('keydown', event => {
  ...
  const popover: any = tagret.closest('.popover')
  if (!popover) return
  if (event.key !== 'Tab') return

  const popoverTrigger = <HTMLElement>getPopoverTrigger(popover.id)
...
document.addEventListener('keydown', event => {
  ...
  const popover: any = target.closest('.popover')
  if (!popover) return
